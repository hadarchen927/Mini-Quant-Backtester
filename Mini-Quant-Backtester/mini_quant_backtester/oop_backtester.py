...
class MABacktester:
    ...
    def calculate_risk_metrics(self):
        ...
        daily_return = self.data['Strategy_Return_Net']

        if daily_return.std() !=0:
            self.sharp_ratio = (daily_return.mean() / daily_return.std()) * np.sqrt(252)
        else:
            self.sharp_ratio = 0.0

        rolling_max = self.data['Cumulative_Strategy'].cummax()
        drawdown = self.data['Cumulative_Strategy'] / rolling_max - 1.0
        self.max_drawdown = drawdown.min()

    def print_performance(self):
        """Print final percentage returns for market and strategy."""
        if not hasattr(self, 'data') or self.data.empty:
            raise ValueError("Backtest data is empty. Run fetch_data() successfully first.")
